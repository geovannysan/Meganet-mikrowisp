const consulta = async (identificado) => {
  ...
  //1204795080
  try {
    const { data } = await axios.post("http://45.224.96.56:3545/PortalApi/GetClientsDetails",
      {        
        "cedula":  identificado,
        "operador": "0999999999"
      })
...
const consultaFact = async (id)=>{
  try {
    const { data } = await axios.get("http://45.224.96.56:3545/PortalApi/GetInvoices/"+id+"/0999999999")
 // console.log(data)
  return data
  } catch (error) {
    return error
